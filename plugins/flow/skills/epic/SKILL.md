...
re-review or a halt. Same glob syntax as `Consequence paths:`, and declared
here rather than built into the driver because per-project path policy
belongs in the document a human signs off. An unusable glob refuses the run:
a line that cannot be applied is fixed here, never silently approximated.
