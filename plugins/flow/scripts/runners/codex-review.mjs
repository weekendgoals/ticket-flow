...
You run in a read-only sandbox with no network, in a detached checkout of the reviewed head: do not fetch, pull, edit, commit or push, and open no pull request. The range and the head below are fixed for you — skip any step of the skill that would look them up over the network. **You cannot serve or render a page**, so where the skill tells you to re-run the fidelity differ, you never can: audit the entry's \`**Compared:**\` table against the design source's own markup instead, and say in your report that the page was not rendered — an audit silent about that reads as a confirmation.
