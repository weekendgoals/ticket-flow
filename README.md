...
| `/flow:quick <description>` | One **small, low-risk** piece of work through the same loop — scope, review, log, pull request — with no epic ceremony. Size- **and risk-gated**: auth, secrets, migrations and other consequential work is routed to `/flow:epic` at any size. Writes a `Q-<n>` ticket into the standing `epics/quick/` epic, then runs it through the same supervisor lane as `/flow:ticket` — a fresh-context worker implements, the session hires the reviewer; `--interactive` runs in-session at the same once-per-session price, one guard watching both commands |
