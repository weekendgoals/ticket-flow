...
- **Tests:** `node --test plugins/flow/scripts/tickets.test.mjs` — expect
  every test passing (`# pass 121`, `# fail 0` as of 2026-09-18; the count
  ...
  `node --test plugins/flow/scripts/check-invariants.test.mjs` (`# pass 30`),
  ...
  `node --test plugins/flow/workflows/run-epic.test.mjs` (`# pass 143`) —
